...
let rec calculateDirectorySize directory =
    ...
    //((selfFiles + (fst childFiles)), [])

// let rec treeWalker directory  =
//     let acc = []
    
//     directory.Children |> List.fold(fun c -> (treeWalker c) :: acc)

//     let acc' = (calculateDirectorySize  directory) :: acc
//     acc'
...
let run () =
    ...
    let result = (snd directorySizes) |> List.filter(fun x -> x < 100000) |> List.sum
