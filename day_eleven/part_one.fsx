...
type Monkey = { id: int; items: int list; ifTrue: int; ifFalse: int; test: int; operation: Operation; inspections: int }
...
let (|Operation|_|) input =
    ...
    if (mSquare.Success) then 
        ...
        Some (plus (int mPlus.Groups.[1].Value))
    elif (mMultiply.Success) then
        Some (multiply (int mMultiply.Groups.[1].Value))        
    else None
...
let monkeyInspection (item: int) (monkey: Monkey) =
    let worry = monkey.operation item
    let afterBored = worry / 3
...
let monkeyFolder (accum: (int * int) list * Monkey list) (monkey: Monkey) =    
...
let run () =
    ...
    let getResultForMonkey' (throws: (int * int) list) (monkey: Monkey) =
        let selectedItems = 
            throws
            |> List.filter(fun t -> snd t = monkey.id)
            |> List.map(fst)
